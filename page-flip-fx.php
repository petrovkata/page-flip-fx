...
*/
	...
	function pageflipfx_get_embed_code($pageflipfx_attributes) {
		...
		$swf_embed = array(
			'width' => $width,
			'height' => $height,
			'text' => trim($pageflipfx_attributes[3]),
	...
	function pageflipfx_filter_content($content) {
		return preg_replace_callback('|\[page-flip-fx\s*(settings="([^\]]+)")?\s*\](.*)\[/page-flip-fx\]|i', 'pageflipfx_get_embed_code', $content);
	}

	function pageflipfx_echo_embed_code($settings_xml_path = '', $div_text = '') {
		echo pageflipfx_get_embed_code(array(2 => $settings_xml_path, 3 => $div_text));
	}
