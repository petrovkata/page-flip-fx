...
/* start global parameters */
	$pageflipfx_params = array(
		'count'	=> 0, // number of Page Flip FX embeds
	);
/* end global parameters */

/* start client side functions */
	function pageflipfx_get_embed_code($pageflipfx_attributes) {
		global $pageflipfx_params;
		$pageflipfx_params['count']++;

		$plugin_dir = get_option('pageflipfx_path');
		if ($plugin_dir === false) {
			$plugin_dir = 'flashxml/page-flip-fx';
		}
		$plugin_dir = trim($plugin_dir, '/');

		$settings_file_name = !empty($pageflipfx_attributes[2]) ? $pageflipfx_attributes[2] : 'settings.xml';
		...
		if (!is_feed()) {
			$embed_code = '<div id="pageflip-fx'.$pageflipfx_params['count'].'">'.$swf_embed['text'].'</div>';
			$embed_code .= '<script type="text/javascript">';
			$embed_code .= "swfobject.embedSWF('{$swf_embed['swf_path']}', 'pageflip-fx{$pageflipfx_params['count']}', '{$swf_embed['width']}', '{$swf_embed['height']}', '9.0.0.0', '', { folderPath: '{$swf_embed['component_path']}'".($settings_file_name != 'settings.xml' ? ", settingsXML: '{$settings_file_name}'" : '')." }, { scale: 'noscale', salign: 'tl', wmode: 'transparent', allowScriptAccess: 'sameDomain', allowFullScreen: true }, {});";
			$embed_code.= '</script>';
		} else {
	...
	function pageflipfx_filter_content($content) {
		return preg_replace_callback('|\[page-flip-fx\s*(settings="([^"]+)")?\s*(width="([0-9]+)")?\s*(height="([0-9]+)")?\s*\](.*)\[/page-flip-fx\]|i', 'pageflipfx_get_embed_code', $content);
	}

	function pageflipfx_echo_embed_code($settings_xml_path = '', $div_text = '', $width = 0, $height = 0) {
		echo pageflipfx_get_embed_code(array(2 => $settings_xml_path, 3 => $div_text, 4 => $width, 6 => $height));
	}

	function pageflipfx_load_swfobject_lib() {
		wp_enqueue_script('swfobject');
	}
/* end client side functions */

/* start admin section functions */
	function pageflipfx_admin_menu() {
		add_options_page('Page Flip FX Options', 'Page Flip FX', 'manage_options', 'pageflipfx', 'pageflipfx_admin_options');
	}

	function pageflipfx_admin_options() {
		  if (!current_user_can('manage_options'))  {
	    wp_die(__('You do not have sufficient permissions to access this page.'));
	  }

	  $pageflipfx_default_path = get_option('pageflipfx_path');
	  if ($pageflipfx_default_path === false) {
	  	$pageflipfx_default_path = 'flashxml/page-flip-fx';
	  }
?>
<div class="wrap">
	<h2>Page Flip FX</h2>
	<form method="post" action="options.php">
		...
		<table class="form-table">
			<tr valign="top">
				...
				<td><input type="text" style="width: 25em;" name="pageflipfx_path" value="<?php echo $pageflipfx_default_path; ?>" /></td>
			</tr>
		</table>
		<input type="hidden" name="action" value="update" />
		<input type="hidden" name="page_options" value="pageflipfx_path" />
		<p class="submit">
